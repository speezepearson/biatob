...
class ApiServer:
    ...
    async def SetTradeDisavowal(self, http_req: web.Request) -> web.Response:
        return proto_response(self._servicer.SetTradeDisavowal(token=self._token_glue.parse_cookie(http_req), request=await parse_proto(http_req, mvp_pb2.SetTradeDisavowalRequest)))
